...
adb_su "pkill -9 frida-server 2>/dev/null; pkill -9 frida 2>/dev/null; true"
adb shell "am force-stop $PKG"
sleep 1

# Verify no frida processes remain
if adb_su "pgrep -f frida" | grep -q .; then
...
timeout 120 frida -U -p "$PID" -l "$SCRIPT_DIR/dump_all_in_one.js" --runtime=qjs \
    2>&1 | tee "$REPO/lib/full_dump/frida_dump.txt"
